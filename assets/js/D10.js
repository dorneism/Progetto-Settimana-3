...
function dice() {
  const rollTheDice = Math.floor(Math.random() * 7);
  return rollTheDice;
}
...
let a = 3;
let b = 17;
const biggerNumber = whoIsBigger(a, b);
...
const example = "My name is Ismaele";
const splitArray = splitMe(example);
...
const results = onlyLetters(myString);
console.log(results);
...
const emailProva = "abcd@Bcd.com";
const emailProva2 = "acbd@bcdcom";
console.log(isThisAnEmail(emailProva));
console.log(isThisAnEmail(emailProva2));
...
function isTodayMyBirthday() {
  const birthday = new Date('06-04-1995');
  const today = new Date();
...
function elementContainer() {
  const selectionContainer = document.getElementById("container");
  return selectionContainer;
}
const idContainer = elementContainer();
...
*/
function selectTd() {
  const tdSelection = document.getElementsByTagName("td")
  return tdSelection;
}
const tdHTML = selectTd();
console.log(tdHTML);
